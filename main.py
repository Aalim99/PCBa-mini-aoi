...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.live_tab = LiveTab(log_path=str(LOG_PATH))
        ...
        self.statusBar().showMessage(
            "Import or load a program in Program Manager, set it active, then calibrate and inspect."
        )
    ...
    def _on_program_activated(self, program, part_sizes):
        ...
        self.statusBar().showMessage(
            f"Active program: {program.get('name', '?')} - press Calibrate, then INSPECT (Space)."
        )
